...
# Pass -Auto to match the orchestrator's declared ServerScripts for levilamina
# (run-e2e-orchestrator.ps1 levilamina.PlatformDef). With -Auto, fetch-server.ps1
...
if (-not $bdsZip) {
    Log "SKIP: BDS server binary unavailable - levilamina E2E not run (fetch-server exited 0 with no download; minecraft.net pocketbedlinks unreachable from this env)"
    exit 0
}
