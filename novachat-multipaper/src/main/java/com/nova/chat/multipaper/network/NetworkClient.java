 ...
 * Reuses novachat-common protocol implementation.
 * 
...
public class NetworkClient {
     ...
     * Uses MULTIPAPER platform type for identification.
     */
    private void sendHandshake() {
        String passwordHash = hashPassword(config.getPassword());
        
        // Include instance ID in username for MultiPaper identification
        String username = config.getUsername();
        if (plugin.getMultiPaperAdapter().isMultiPaper()) {
            username = username + "@" + plugin.getMultiPaperAdapter().getInstanceId();
        }
