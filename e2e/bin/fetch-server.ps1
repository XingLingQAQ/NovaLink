...
# fetch-server.ps1 lives at <repo>/e2e/bin/. versions.lock.ps1 lives at
# <repo>/e2e/. So repoRoot = three levels up from the script: bin -> e2e -> repo.
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$e2eDir = Split-Path -Parent $scriptDir
. (Join-Path $e2eDir 'versions.lock.ps1')
