...
/**
 * Registers NovaChat commands using Quilted Fabric API's CommandRegistrationCallback
 ...
 */
public class QuiltCommandRegistrar {
    private static final Logger LOGGER = LoggerFactory.getLogger(QuiltCommandRegistrar.class);
    ...
    public static void registerCommands(CommandManager manager) {
        ...
        // Register with Quilted Fabric API's command system (compatible with Fabric API)
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            registerNovaChatCommand(dispatcher);
            LOGGER.info("NovaChat commands registered (Quilt)");
        });
    }
    ...
    private static int executeSubCommand(CommandContext<CommandSourceStack> ctx, String subCommand, String[] args) {
        ...
        QuiltPlatform platform = NovaChatQuilt.getInstance().getPlatform();
        boolean isAdmin = source.hasPermission(2);
        com.nova.chat.mod.platform.CommandContext cmdContext = 
            new QuiltCommandContext(player.getUUID(), player.getName().getString(), platform, isAdmin, source);
    ...
    private static int executeReload(CommandContext<CommandSourceStack> ctx) {
        ...
        NovaChatQuilt.getInstance().reloadConfig();
    ...
    private static int executeDebug(CommandContext<CommandSourceStack> ctx) {
        ...
        NovaChatQuilt instance = NovaChatQuilt.getInstance();
        ...
        source.sendSuccess(() -> Component.literal("§7Platform: §fQuilt"), false);
