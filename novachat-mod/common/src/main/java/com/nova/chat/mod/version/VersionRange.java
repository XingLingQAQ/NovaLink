...
public enum VersionRange {
    ...
    /**
     * Modern versions: 1.20.x through 1.21.x and calendar-line 26.x+
     * These versions use Component API and modern chat events.
     * maxMajor/maxMinor are soft upper bounds for display; contains() also
     * accepts major >= 26 (2026+ calendar versioning).
     */
    MODERN_1_20_PLUS("1.20-1.21/26.x", 1, 20, 26, 99),
    ...
    public boolean contains(int major, int minor) {
        ...
        // Calendar-line versions (26.x+) are always modern for this adapter set
        if (this == MODERN_1_20_PLUS && major >= 26) {
            return true;
        }
    ...
    public static VersionRange forVersion(int major, int minor) {
        // 26.x+ calendar versions map to modern adapter
        if (major >= 26) {
