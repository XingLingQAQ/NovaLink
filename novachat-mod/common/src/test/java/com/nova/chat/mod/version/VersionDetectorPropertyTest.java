...
class VersionDetectorPropertyTest {
    ...
    void specificVersionsAreCorrectlyClassified() {
        ...
        assertThat(new VersionDetector("1.21.11").getVersionRange()).isEqualTo(VersionRange.MODERN_1_20_PLUS);
        // Calendar-line versions (2026+)
        assertThat(new VersionDetector("26.1").getVersionRange()).isEqualTo(VersionRange.MODERN_1_20_PLUS);
        assertThat(new VersionDetector("26.2").getVersionRange()).isEqualTo(VersionRange.MODERN_1_20_PLUS);
        assertThat(new VersionDetector("26.1.2").isSupported()).isTrue();
    }

    @Property(tries = 50)
    @Report(Reporting.GENERATED)
    void calendarLineVersionsAreModern(
            @ForAll @IntRange(min = 26, max = 30) int major,
            @ForAll @IntRange(min = 0, max = 12) int minor,
            @ForAll @IntRange(min = 0, max = 10) int patch) {
        ...
        assertThat(detector.getVersionRange())
                .as("Calendar version %s should be MODERN", version)
                .isEqualTo(VersionRange.MODERN_1_20_PLUS);
        assertThat(detector.isSupported()).isTrue();
        assertThat(VersionRange.forVersion(major, minor)).isEqualTo(VersionRange.MODERN_1_20_PLUS);
    }

    @Example
    void versionRangeDescriptionIsNotEmpty() {
        VersionDetector detector = new VersionDetector("1.21.11");
        assertThat(detector.getVersionRangeDescription()).isNotEmpty();
        assertThat(detector.getVersionRangeDescription()).contains("26");
        assertThat(detector.toString()).contains("1.21.11");
    }
}
