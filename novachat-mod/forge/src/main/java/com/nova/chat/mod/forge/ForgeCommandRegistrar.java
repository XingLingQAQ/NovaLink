...
/**
 * Registers NovaChat commands using Forge's RegisterCommandsEvent
 * 
...
public class ForgeCommandRegistrar {
    private static final Logger LOGGER = LoggerFactory.getLogger(ForgeCommandRegistrar.class);
    ...
    public static void registerCommands(CommandManager manager) {
        ...
        // Register with Forge's command system using RegisterCommandsEvent
        MinecraftForge.EVENT_BUS.register(new CommandEventHandler());
    }

    
    /**
     * Event handler class for RegisterCommandsEvent
     */
    ...
    private static int executeSubCommand(CommandContext<CommandSourceStack> ctx, String subCommand, String[] args) {
        ...
        ForgePlatform platform = NovaChatForge.getInstance().getPlatform();
        boolean isAdmin = source.hasPermission(2);
        com.nova.chat.mod.platform.CommandContext cmdContext = 
            new com.nova.chat.mod.platform.CommandContext(player.getUUID(), player.getName().getString(), platform, isAdmin);
    ...
    private static int executeReload(CommandContext<CommandSourceStack> ctx) {
        ...
        NovaChatForge.getInstance().reloadConfig();
    ...
    private static int executeDebug(CommandContext<CommandSourceStack> ctx) {
        ...
        NovaChatForge instance = NovaChatForge.getInstance();
        ...
        source.sendSuccess(() -> Component.literal("§7Platform: §fForge"), false);
