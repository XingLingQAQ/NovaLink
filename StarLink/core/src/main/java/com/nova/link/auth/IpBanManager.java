...
public class IpBanManager {
    ...
    /**
     * Removes expired bans from the internal map.
     * Safe to call periodically from a maintenance task.
     *
     * @return the number of expired bans removed
     */
    public int cleanupExpiredBans() {
        long now = System.currentTimeMillis();
        int removed = 0;
        for (Map.Entry<String, Long> entry : banExpirations.entrySet()) {
            Long expiration = entry.getValue();
            if (expiration != null && now >= expiration) {
                if (banExpirations.remove(entry.getKey(), expiration)) {
                    removed++;
                }
            }
        }
        if (removed > 0) {
            logger.debug("Cleaned up {} expired IP bans", removed);
        }
        return removed;
    }

    /**
     * @return number of currently tracked (not necessarily active) ban entries
     */
    public int getTrackedBanCount() {
        return banExpirations.size();
    }
}
