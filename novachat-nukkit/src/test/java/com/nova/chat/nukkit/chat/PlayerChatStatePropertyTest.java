...
/**
 * Property-based tests for shared {@link PlayerChannelState} from the Nukkit module.
 */
class PlayerChatStatePropertyTest {
    ...
    /** Non-blank channel ids (PlayerChannelState rejects blank). */
    @Provide
    ...
    void nukkitPlayerChatStateCreation(@ForAll("uuids") UUID playerId,
                                        @ForAll("channelIds") String channel) {
        // Nukkit version doesn't use currentServer field (single server)
        PlayerChannelState state = new PlayerChannelState(playerId, channel, ChatMode.HYBRID);
