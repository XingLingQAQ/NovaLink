...
/**
 * Async network client for Folia using Folia's scheduler for all operations.
 * All network operations are performed asynchronously to avoid blocking region threads.
 * 
 * Requirements: 2.2
 */
public class AsyncNetworkClient {
     ...
     * Uses Folia's async scheduler for the connection process.
     *
     * Requirements: 2.2
     *
    ...
    public CompletableFuture<Boolean> connect(String host, int port) {
        ...
        // Run connection on async thread to avoid blocking any region
        scheduler.runAsync(() -> {
            ...
            bootstrap.group(workerGroup)
                ...
                .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, config.getNetworkTimeout())
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel ch) {
                        ...
                        // Handler
                        pipeline.addLast("handler", new AsyncClientChannelHandler(AsyncNetworkClient.this));
                    }
                });
    ...
    /**
     * Sends a packet to the backend asynchronously.
     ...
     * Called from Netty's event loop thread.
     *
    ...
    void handlePacket(Packet packet) {
        ...
        if (handler != null) {
            // Handle packet on async thread to avoid blocking Netty
            scheduler.runAsync(() -> handler.accept(packet));
        } else {
     ...
     * Uses Folia's async scheduler for the delay.
     */
    private void scheduleReconnect() {
        ...
        // Use Folia's async scheduler for delayed reconnection
