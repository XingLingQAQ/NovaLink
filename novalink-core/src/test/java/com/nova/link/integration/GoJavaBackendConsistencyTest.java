...
class GoJavaBackendConsistencyTest {
    ...
    void testChannelActionResponseConsistency() throws Exception {
        ...
        // Send channel join action (production ChannelActionHandler requires playerId)
        ChannelActionPacket joinAction = new ChannelActionPacket(
            ChannelAction.JOIN,
            "test-channel",
            ""
        );
        joinAction.addExtra("playerId", UUID.randomUUID().toString());
        joinAction.addExtra("playerName", "ChannelPlayer");
