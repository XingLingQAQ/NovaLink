...
public class NovaChatCommand extends Command implements CommandExecutor {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (subCommand == null) {
            ...
            sendError(sender, I18n.tr(playerId, "chat.command.unknown", subCommandName));
            sendMessage(sender, I18n.tr(playerId, "chat.command.unknown_hint", label));
            return true;
        }
        ...
        if (!subCommand.hasPermission(sender)) {
            UUID playerId = sender instanceof Player ? ((Player) sender).getUniqueId() : null;
            sendError(sender, I18n.tr(playerId, "chat.command.no_permission_code"));
            return true;
        }
