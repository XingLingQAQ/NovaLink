...
/**
 * Property-based tests for NovaChatConfig in Velocity module.
 * 
 * Note: These tests use a mock config approach since Velocity's Toml
 * requires the toml4j library at runtime. We test the configuration parsing logic patterns.
 */
class NovaChatConfigPropertyTest {

    /**
     * Simple mock config for testing without Toml dependencies.
     */
    static class MockConfig {
        ...
        public MockConfig() {
            ...
            data.put("backend.port", 8888L);
            data.put("backend.username", "");
            data.put("backend.password", "");
            data.put("backend.reconnect-delay", 5L);
    ...
    void defaultPortShouldBe8888() {
        MockConfig mock = new MockConfig();
        int port = mock.getLong("backend.port", 8888L).intValue();
    ...
    void portShouldBeInValidRange(@ForAll @IntRange(min = 1, max = 65535) int port) {
        MockConfig mock = new MockConfig();
        mock.set("backend.port", (long) port);
        int configPort = mock.getLong("backend.port", 8888L).intValue();
    ...
    void reconnectDelayShouldBePositive(@ForAll @IntRange(min = 1, max = 300) int delay) {
        MockConfig mock = new MockConfig();
        mock.set("backend.reconnect-delay", (long) delay);
        int configDelay = mock.getLong("backend.reconnect-delay", 5L).intValue();
